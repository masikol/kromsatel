...
primers=''
genome=''
outfile='./corona-amplicons.fasta'
seqkit='seqkit'

# Parse command line arguments
while getopts ":p:g:o:s:h" opt
do
  case "${opt}" in
    p) primers="${OPTARG}"
    ;;
...
if [[ ${HELP} == 1 ]]; then
    ...
    echo -e "   just like \`amplicons-db/nCoV-2019_amplicons.fasta\`.\n"
    echo "Options:"
    echo " -h -- print help message and exit;"
    echo " -p -- CSV file with primers  (mandatory);"
    echo " -g -- fasta file with genome (mandatory);"
    echo " -o -- output fasta file with amplicons;"
    echo "       default value: \`./corona-amplicons.fasta\`"
    echo " -s -- path to \`seqkit\` executable"
    echo -e "       (if \`seqkit\` is in your PATH, just omit this option);\n"
    echo "Usage:"
    echo -e "  bash $0 <-p primers_csv> <-g genome_fasta> [-o outout_fasta] [-s seqkit_path]\n"
    echo "Examples:"
    echo " Case 1. \`seqkit\` is in PATH"
    echo "  bash $0 -p my_primers.csv -g Wuhan-Hu-1-compele-genome.fasta -o amplicons-db/my_amplicons.fasta"
    echo " Case 2. \`seqkit\` is not in PATH"
    echo "  bash $0 -p my_primers.csv -g Wuhan-Hu-1-compele-genome.fasta -o amplicons-db/my_amplicons.fasta -s /home/me/seqkit/bin/seqkit"
...
echo -e "make-amplicons-fasta.sh Version ${VERSION}\n"
...
for (( i=0; i<${#primer_seqs[@]}; i+=2 )); do
    ...
    seq=`seqkit_amplicon_get_seq "${genome}" "${seqkit}" "${forward}" "${reverse}"`
    echo -en ">${title}\n${seq}\n" >> "${outfile}"
